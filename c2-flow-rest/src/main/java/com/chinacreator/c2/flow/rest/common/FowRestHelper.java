...
public class FowRestHelper {
	 ...
	 * @return
	 */
	public static WfOperator getWfOperator(WfBaseRequest actionRequest)
			throws Exception {

		boolean isDebug = "true".equals(ConfigManager.getInstance().getConfig(
				"c2.flow.rest.debug")) ? true : false;
		...
		// 如果开启了调试模式从参数中取当前用户
		if (StringUtils.hasText(actionRequest.getCurrentLoginUserId())
				&& isDebug) {
		...
		if (!StringUtils.hasText(wfOperator.getUserId())) {
			throw new FlowUnauthorizedException(
					"Not logged in without permission to operate.");
		}
	 ...
	 * @return
	 */
	public static WfOperator getWfOperator(String currentLoginUserId)
			throws Exception {
		...
		// 如果开启了调试模式从参数中取当前用户
		if (StringUtils.hasText(currentLoginUserId) && isDebug) {
			wfOperator.setUserId(currentLoginUserId);
		} else if (null != subject && StringUtils.hasText(subject.getId())) {
			wfOperator.setUserId(subject.getId());
		}
