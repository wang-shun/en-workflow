...
///////// modify by yicheng.yang begine 已办理环节和已经办理的流经线的颜色为绿色
var HIGHLIGHT_ACTIVATED_FLOW_COLOR = Color.Green;
var HIGHLIGHT_EXECUTED_ACTIVITY = Color.Green;
///////// modify by yicheng.yang end 已办理环节和已经办理的流经线的颜色为绿色
...
// TODO: generate polylineId or do something!!
	...
	*/
	...
	// 已办理的环节需要将颜色变成绿色 modify by yicheng。yang
	highLightExecutedActivity: function(activityId, pId){
		...
		var contextObject = shape.data("contextObject");
		if (contextObject)
			console.log("--> highLightExecutedActivity: ["+contextObject.getProperty("type")+"], activityId: " + contextObject.getId());
		else
			console.log("--> highLightExecutedActivity: ", shape, shape.data("contextObject"));
		
		shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
		shape.attr("stroke", HIGHLIGHT_EXECUTED_ACTIVITY);
	},
	...
	highLightFlow: function(flowId){
		...
		// yyc modifyed begine 改颜色
//		st.attr("stroke", HIGHLIGHT_COLOR);
		st.attr("stroke", HIGHLIGHT_ACTIVATED_FLOW_COLOR);
		// yyc modifyed end 改颜色
		
		var withArrowHead = shapeFlow.data("withArrowHead");
		if (withArrowHead)
			// yyc modifyed begine 改颜色
			//st[1].attr("fill", HIGHLIGHT_COLOR);
			st[1].attr("fill", HIGHLIGHT_ACTIVATED_FLOW_COLOR);
			// yyc modifyed end 改颜色
