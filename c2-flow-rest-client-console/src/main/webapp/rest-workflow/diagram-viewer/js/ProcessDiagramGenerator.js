...
var ProcessDiagramGenerator = {	
	...
	setHighLights: function(highLights){
		if (highLights.processDefinitionId == undefined || highLights.processInstanceId == undefined) {
		...
		// yyc add 已完成活动高亮显示
		for (var i in highLights.activitiesExecuted) {
			var activityId = highLights.activitiesExecuted[i];
			processDiagramCanvas.highLightExecutedActivity(activityId, highLights.processInstanceId);
		}
		
		// TODO: remove highLightes from all activities before set new highLight
		for (var i in highLights.activities) {
			var activityId = highLights.activities[i];
			processDiagramCanvas.highLightActivity(activityId, highLights.processInstanceId);
		}
