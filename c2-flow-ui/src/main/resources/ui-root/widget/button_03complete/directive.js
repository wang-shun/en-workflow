var completeButton = function (controlState) {
	this.state=controlState;
};
completeButton.prototype = {
	setDisabled:function(disabled){
		this.state.disabled=disabled;
	},
	setVisible:function(visible){
		this.state.visible=visible;
	}
}
directives.directive('completeButton', ['$http','Messenger','FormContainerFactory',function($http,Messenger,FormContainerFactory ){
	return {
		link:function(scope, element, attrs){
			var userId =  scope.subject.principal;
			scope.completeTask = function(taskId, variables) {
				if(!taskId){
					Messenger.error("任务ID为空，请设置参数$params.taskId");
					return;
				}
				if(!variables){
					Messenger.error("流程变量为空，如果流程定义有变量，请设置参数$params.variables={...}");
				}
				$http.get("ws/operateTaskTmp",{
					params:{
						taskId:taskId,
						action:'complete',
						userToDelegateTo:1,
						variables:variables,
						wfOperator:{
							userId:userId,
							businessData:{}
					    }
					}
				}).success(function(data){
					if(data.result_out.result=='200'){
						Messenger.post({
						    'message': "签收成功！",
						    'type': 'success',
							});
					}			
				});
			}
			var form = FormContainerFactory.findForm(scope);
			if (angular.isUndefined(scope.$state[attrs.id])) {
				scope.$state[attrs.id] = {};
			}
			form[attrs.id] = new completeButton(scope.$state[attrs.id]);
		}
	};
}]);
